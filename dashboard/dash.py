...
def remove_empty_params(params):
    """ Elimina parámetros vacíos del diccionario """
    return {k: v for k, v in params.items() if v not in [None, "", 0.0]}
...
if st.button("Cargar Productos"):
    productos = get_data("productos/productos")
    st.write(pd.DataFrame(productos))

if st.button("Cargar Clientes"):
    clientes = get_data("clientes/clientes")
    st.write(pd.DataFrame(clientes))

if st.button("Cargar Proveedores"):
    proveedores = get_data("proveedores/proveedores")
    st.write(pd.DataFrame(proveedores))

if st.button("Cargar Ventas"):
    ventas = get_data("ventas/ventas")
    df_ventas = pd.DataFrame(ventas)

    if "sucursal_id" in df_ventas.columns:
        df_ventas["sucursal_id"] = pd.to_numeric(df_ventas["sucursal_id"], errors="coerce").fillna(0).astype("int")
    st.write(df_ventas)

if st.button("Cargar Logística"):
    logistica = get_data("logistica/logistica")
...
categoria_producto = st.text_input("Categoría", key="categoria_producto")
...
estados_posibles = ['Retrasado', 'Entregado', 'En tránsito', 'Cancelado']
estado_envio = st.selectbox("Estado de Envío", estados_posibles, key="estado_envio")
